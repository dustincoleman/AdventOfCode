...
namespace AdventOfCode2020
{
    ...
    {
        public static void FlipLR(this bool[,] array)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                {
                    bool tempBool = array[i, j];
                    array[i, j] = array[i, offset - j];
                    array[i, offset - j] = tempBool;
                }
            }
        }

        public static void FlipTB(this bool[,] array)
        {
            int size = array.GetLength(0);
            int halfSize = size / 2;
            int offset = size - 1;
            ...
            for (int j = 0; j < size; j++)
            {
                ...
                {
                    bool tempBool = array[i, j];
                    array[i, j] = array[offset - i, j];
                    array[offset - i, j] = tempBool;
                }
            }
        }

        public static void Rotate(this bool[,] array)
        {
            int size = array.GetLength(0);
            int halfSize = size / 2;
            int offset = size - 1;
            ...
            for (int i = 0; i < halfSize; i++)
            {
                for (int j = i; j < offset - i; j++)
                {
                    ...
                    // Move left to top
                    array[i, j] = array[offset - j, i];

                    // Move right to left
                    array[offset - j, i] = array[offset - i, offset - j];

                    // Move top to right
                    array[offset - i, offset - j] = array[j, offset - i];

                    // Assign temp to right
                    array[j, offset - i] = tempBool;
                }
            }
        }
    }
