...
{
    [Serializable]
    internal class ImmutableGrid2 : IGrid2
    {
        int boundsX;
        int boundsY;
        object[,] data;

        internal ImmutableGrid2(IGrid2 source)
        {
            this.boundsX = source.Bounds.X;
            this.boundsY = source.Bounds.Y;
            this.data = new object[this.boundsX, this.boundsY];

            foreach (Point2 point in source.Points)
            {
                data[point.X, point.Y] = source[point];
            }
        }
